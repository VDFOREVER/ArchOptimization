...
case $xz in
    ...
    case $soft in
        ...
        case $st in
            y) sudo pacman -S base-devel git gvfs ccache gamemode lib32-gamemode rng-tools ; sudo systemctl enable --now rngd; sudo pacman -S dbus-broker ; sudo systemctl enable --now dbus-broker.service ; sudo systemctl enable fstrim.timer ; sudo fstrim -v /;;
            n) sudo pacman -S base-devel git gvfs ccache gamemode lib32-gamemode ; git clone https://aur.archlinux.org/ananicy.git ; cd ananicy ; makepkg -sric ; sudo systemctl enable --now ananicy ; cd .. ; sudo rm -R ananicy ;
